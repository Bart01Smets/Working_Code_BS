...
run_sir_binomial <- function(initial_state, 
  ...
  # Run simulation loop over time
  for (i_day in times[-1]) {
    if (bool_stochastic_beta) {
      # Step 1: Generate individual-level transmission rates
      beta_individual <- rnorm(parameters$pop_size, mean = parameters$beta, sd = parameters$sigma)
      
      # Step 2: Calculate aggregated transmission rate for current state
      if (Ni > 0) {
        beta_agg <- mean(beta_individual[1:Ni])  # Average of \( \beta_i \) for infected individuals
      } else {
        beta_agg <- parameters$beta  # Default to global beta if no infections
      }
    } else {
      beta_agg <- parameters$beta
    }
    
    # Calculate optimal action
    a_t <- a_function(parameters$alpha, beta_agg, 
    ...
    new_infections <- update_function(Ns, prob = beta_agg * a_t^2 * Ni / parameters$pop_size)
    ...
    if (parameters$scenario == "laissez-faire") {
      d_lambda_s <- rho_plus_delta * lambda_s - 
        (u_t + (lambda_i - lambda_s) * beta_agg * a_t^2 * Ni / parameters$pop_size)
      d_lambda_i <- rho_plus_delta * lambda_i - 
        (u_t + parameters$alpha * (lambda_i - lambda_s) * beta_agg * a_t^2 * Ns / parameters$pop_size) - 
        parameters$gamma * (parameters$kappa + lambda_i)
    } else if (parameters$scenario == "optimal-policy") {
      d_lambda_s <- rho_plus_delta * lambda_s - 
        u_t - (lambda_i - lambda_s) * beta_agg * a_t^2 * Ni / parameters$pop_size
      d_lambda_i <- rho_plus_delta * lambda_i - 
        u_t - (lambda_i - lambda_s) * beta_agg * a_t^2 * Ns / parameters$pop_size + 
        parameters$gamma * (parameters$kappa + lambda_i)
    }
