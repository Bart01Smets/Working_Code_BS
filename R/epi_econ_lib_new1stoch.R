...
compare_sim_output <- function(output_experiments, output_deterministic,
  ...
  for (i_state in sel_states) {
    ...
    # keep dims when only 1 experiment
    var_cube   <- output_all[, , i_state, drop = FALSE]
    var_matrix <- matrix(var_cube, nrow = dim(output_all)[1])
    time_vec   <- 0:(ncol(var_matrix) - 1)
    
    # quantile band (always defined; zero-width if 1 run)
    ...
    # mean and (if >1 run) 95% CI of the mean
    n_sim   <- nrow(var_matrix)
    ci_mean <- colMeans(var_matrix, na.rm = TRUE)
    if (n_sim > 1) {
      ...
      ci_lo_meanband <- ci_mean
      ci_hi_meanband <- ci_mean
    }
    ...
    # quantile band
    polygon(c(time_vec, rev(time_vec)),
            c(ci_upper, rev(ci_lower)),
            col = alpha("lightblue", 0.4), border = NA)
    
    # mean-CI band only if >1 run
    if (n_sim > 1) {
      polygon(c(time_vec, rev(time_vec)),
              c(ci_hi_meanband, rev(ci_lo_meanband)),
  ...
  if (nrow(output_summary) > 1) {
    peak_ci <- ci_mean(peak_times)
    cat(sprintf("Mean Peak Time: %.1f [%.1f, %.1f]\n",
