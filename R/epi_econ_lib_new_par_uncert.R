...
# --- Parameter sampler (no I/O, no globals) ---
...
run_psa_full <- function(initial_state, times, base_parameters,
                         ...
                         update_function = get_transitions_stochastic) {
  set.seed(base_parameters$rng_seed)
  # We will return a single 3D array pooling ALL sims across ALL draws:
  #   dim = [n_param_draws * n_stoch, length(times), n_vars]
  # We first run one sim to learn the variable names and dims.
...
plot_dual_uncertainty_bands <- function(baseline_output_all,   # 3D array from run_experiments()$output_all
                                        ...
                                        deterministic_df,      # data.frame from run_sir_binomial(... deterministic ...)
                                        var = "Ni",            # which variable to plot: "Ni", "a_t", "TotalCost", etc.
                                        title = NULL) {
  
  # Extract matrices [runs x time] for chosen variable
  base_mat <- baseline_output_all[, , var, drop = TRUE]
  psa_mat  <- psa_output_all[, , var, drop = TRUE]
  
  # Time vector (assumes time starts at 0 and increments by 1)
  time_vec <- 0:(ncol(base_mat) - 1)
  
  # Helper to compute ribbons
  ...
  base_sum <- ribbon_summ(base_mat)  # stochasticity only
  psa_sum  <- ribbon_summ(psa_mat)   # stochasticity + parameter uncertainty
  ...
  ylab_txt <- switch(var,
                     ...
                     var)
  if (is.null(title)) {
    title <- paste("Dual Uncertainty Bands for", ylab_txt)
  }
  
  ggplot() +
    # Outer ribbon = parameter + stochastic uncertainty (PSA)
    geom_ribbon(data = psa_sum,
                aes(x = time, ymin = q025, ymax = q975),
                alpha = 0.25) +
    # Inner ribbon = stochastic-only uncertainty (fixed params)
