...
run_sir_binomial <- function(initial_state, 
  ...
  for (i_day in times[-1]) {
    if (bool_stochastic_beta) {
      # Generate individual-specific beta_t values
      beta_t_individuals <- pmax(0.0001, rnorm(Ns + Ni, mean = parameters$beta, sd = parameters$sigma))
    } else {
      # Uniform beta_t for all individuals
      beta_t_individuals <- rep(parameters$beta, Ns + Ni)
    }
    
    # Average beta_t for population-level calculations
    beta_t <- mean(beta_t_individuals)
    ...
    # Calculate transitions using individual-specific beta_t values
    new_infections <- sum(rbinom(n = Ns, size = 1, prob = beta_t_individuals[1:Ns] * a_t^2 * Ni / parameters$pop_size))
    ...
    # Update costs
    HealthCost <- HealthCost + fx_per_capita * exp(-rho_plus_delta * i_day) * parameters$kappa * Nd
    ...
    Nr <- Nr + dNr
    Nd <- Nd + dNd
    lambda_s <- lambda_s + rho_plus_delta * lambda_s - u_t - (lambda_i - lambda_s) * beta_t * a_t^2 * Ni / parameters$pop_size
    lambda_i <- lambda_i + rho_plus_delta * lambda_i - u_t - (lambda_i - lambda_s) * beta_t * a_t^2 * Ns / parameters$pop_size + parameters$gamma * (parameters$kappa + lambda_i)
    
    # Record states
    states_out[i_day + 1, ] <- c(Ns, Ni, Nr, Nd, lambda_s, lambda_i, HealthCost, SocialActivityCost, 
                                 HealthCost + SocialActivityCost, a_t, u_t, Rt)
  }
