...
compare_sim_output <- function(output_experiments, output_deterministic,
  ...
  output_summary_deterministic <- output_deterministic[nrow(output_sim_deterministic),]
  ...
  took_off <- with(output_experiments$output_summary,
                   (Nr + Nd + Ni) >= fadeout_threshold)
  summary_filtered <- output_experiments$output_summary[took_off, , drop = FALSE]
  
  # Stochastic vector of total costs at horizon
  stoch_costs <- summary_filtered$TotalCost
  
  # Deterministic total cost at horizon
  det_cost <- tail(output_sim_deterministic$TotalCost, 1)
  
  # Quick stats for the paper
  stoch_mean <- mean(stoch_costs, na.rm = TRUE)
  stoch_ci    <- suppressWarnings(confintr::ci_mean(stoch_costs)$interval)
  stoch_q025  <- quantile(stoch_costs, 0.025, na.rm = TRUE)
  stoch_q975  <- quantile(stoch_costs, 0.975, na.rm = TRUE)
  
  cat("\n--- TOTAL COST (per capita) DISTRIBUTION ---\n")
  cat(sprintf("Stochastic mean: %.0f  (95%% CI: [%.0f, %.0f])\n", stoch_mean, stoch_ci[1], stoch_ci[2]))
  cat(sprintf("Stochastic 95%% quantiles: [%.0f, %.0f]\n", stoch_q025, stoch_q975))
  cat(sprintf("Deterministic total cost: %.0f\n", det_cost))
  cat(sprintf("Mean diff (Det - Stoch mean): %.2f%%\n", 100*(det_cost - stoch_mean)/det_cost))
  
  # Plot
  df_cost <- data.frame(TotalCost = stoch_costs)
  print(
  ggplot(df_cost, aes(x = TotalCost)) +
    geom_histogram(aes(y = ..density..), bins = 40, alpha = 0.35) +
    geom_density(linewidth = 1) +
    geom_vline(xintercept = det_cost, linetype = "dashed") +
    geom_vline(xintercept = stoch_mean, linewidth = 0.8) +
    labs(
      title = "Total Cost (per capita): Stochastic Distribution vs Deterministic",
      subtitle = sprintf("Deterministic = %.0f, Stochastic mean = %.0f", det_cost, stoch_mean),
      x = "Total Cost (per capita)",
      y = "Density"
    ) +
    theme_minimal()
  )
  # --- FADE-OUT PROBABILITY & TIMING -----------------------------------------
  library(ggplot2)
  ...
  cat(sprintf("IQR (time-to-extinction): %s\n",
              ifelse(all(is.na(first_ext_time)), "NA",
