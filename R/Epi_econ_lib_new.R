...
#   op <- par(cex.lab = 1.4, cex.axis = 1.2)   # bigger axis titles & numbers
...
#   
#   # =======================================================================================
#   # MODE A: deterministic (no-carry) vs deterministic (carry)  — no stochastic distributions
#   # =======================================================================================
#   if (isTRUE(compare_carry)) {
#     stopifnot(!is.null(output_det_nocarry), !is.null(output_det_carry))
#     
#     # 1) Costs: side-by-side bars for final Health/Social/Total costs
#     last_nc <- output_det_nocarry[nrow(output_det_nocarry), c("HealthCost","SocialActivityCost","TotalCost")]
#     last_c  <- output_det_carry  [nrow(output_det_carry  ), c("HealthCost","SocialActivityCost","TotalCost")]
#     
#     cost_mat <- rbind(NoCarry = as.numeric(last_nc), Carry = as.numeric(last_c))
#     colnames(cost_mat) <- c("HealthCost","ActivityCost","TotalCost")
#     
#     y_lim <- range(cost_mat, na.rm = TRUE)
#     barplot(t(cost_mat), beside = TRUE, ylim = y_lim, las = 2,
#             ylab = "Per-capita cost", main = paste(plot_tag, "(final costs)"))
#     legend("topleft", fill = gray(c(0.5, 0.8)), legend = c("No carry","Carry"), bty = "n")
#     
#     # Print summary table to console-style output
#     print_out <- data.frame(
#       output = c('Health Cost (per capita)','Activity Cost (per capita)','Total Cost (per capita)'),
#       deterministic_NoCarry = c(last_nc$HealthCost, last_nc$SocialActivityCost, last_nc$TotalCost),
#       deterministic_Carry   = c(last_c$HealthCost,  last_c$SocialActivityCost,  last_c$TotalCost)
#     )
#     names(print_out)[1] <- plot_tag
#     print(paste('SUMMARY STATISTICS:', plot_tag))
#     print(print_out)
#     
#     # 2) Time series: plot both deterministic paths
#     all_cols   <- intersect(colnames(output_det_nocarry), colnames(output_det_carry))
#     sel_states <- setdiff(all_cols, c("TotalCost","Ns"))
#     
#     time_vec <- seq_len(nrow(output_det_nocarry)) - 1
...
#       
#       lines(time_vec, output_det_nocarry[, i_state], col = "black", lwd = 2, lty = 2)
#       lines(time_vec, output_det_carry  [, i_state], col = "blue",  lwd = 2)
#       
#       legend("topright",
#              legend = c("Deterministic (no carry)", "Deterministic (carry)"),
#              col = c("black","blue"), lwd = 2, lty = c(2,1), bty = "n")
#     }
#     
#     return(invisible(NULL))
#   }
#   
#   # =========================================================================
#   # MODE B (default): stochastic vs deterministic  — your original behavior
#   # =========================================================================
#   output_summary <- output_experiments$output_summary
#   output_all     <- output_experiments$output_all
#   
...
#   }
#   
#   # (keeps your existing console summaries that follow…)
#   # ... leave the rest of your original Stochastic-vs-Deterministic code here ...
# 
#   
