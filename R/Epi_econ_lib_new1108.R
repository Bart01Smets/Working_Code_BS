...
a_function <- function(Ni, Ns, parameters, beta_t = NULL) {
  if (isTRUE(parameters$bool_regular_sird)) return(1)
  if (is.null(beta_t)) beta_t <- parameters$beta
  ...
  multiplier <- beta_t * parameters$pi * parameters$v * Ns_prop * Ni_prop
  num_sum <- Ns_prop + Ni_prop
  discrim <- num_sum^2 + 4 * multiplier * num_sum
  denom <- 2 * multiplier
...
# # Map legacy flags to beta_process (optional/back-compat)
# resolve_beta_process <- function(parameters, bool_stochastic_beta) {
#   if (!is.null(parameters$beta_process)) return(parameters$beta_process)
#   if (isTRUE(parameters$bool_beta_wiener)) return("wiener")
#   if (isTRUE(bool_stochastic_beta)) return("iid_normal")
#   return("fixed")
# }

resolve_beta_process <- function(parameters, bool_stochastic_beta = FALSE) {
  # single source of truth:
  # "fixed", "iid_normal", "wiener", "gbm", "state_dependent"
  #proc <- parameters$beta_process %||% "fixed"  # define %||% as a null-coalescing op if you like
  proc <- if (is.null(parameters$beta_process)) "fixed" else parameters$beta_process
  
  return(proc)
}

resolve_beta_process <- function(parameters, bool_stochastic_beta = FALSE) {
  if (!bool_stochastic_beta) return("fixed")
  return(if (is.null(parameters$beta_process)) "iid_normal" else parameters$beta_process)
}



# Keep beta within [beta_min, beta_max]
.clamp_beta <- function(b, p) pmin(pmax(b, p$beta_min), p$beta_max)

# One-step evolution of beta_t given chosen process.
# Uses only *known* info at start of the day (Ns, Ni). For state dependence,
# diffusion scales with prevalence: sigma_t = beta_sigma * (1 + k * Ni_prop / (beta_ref + Ni_prop))
evolve_beta <- function(beta_t, parameters, Ns, Ni, bool_stochastic_beta) {
  proc <- resolve_beta_process(parameters, bool_stochastic_beta)
  mu   <- parameters$beta_mu
  sig  <- parameters$beta_sigma
  
  # state-dependent sigma (prevalence-based)
  if (proc == "state_dependent") {
    Ni_prop <- Ni / parameters$pop_size
    sig <- sig * (1 + parameters$beta_k_state * (Ni_prop / (parameters$beta_ref + Ni_prop)))
  }
  
  if (proc == "fixed") {
    return(.clamp_beta(parameters$beta, parameters))
  }
  
  if (proc == "iid_normal") {
    b <- rnorm(1, mean = parameters$beta, sd = sig)
    return(.clamp_beta(b, parameters))
  }
  
  if (proc == "wiener") {
    # additive BM: b_{t+1} = b_t + mu*dt + sig*sqrt(dt)*Z, dt=1
    b <- beta_t + mu + sig * rnorm(1)
    return(.clamp_beta(b, parameters))
  }
  
  if (proc == "gbm") {
    # geometric BM: b_{t+1} = b_t * exp((mu - 0.5*sig^2)*dt + sig*sqrt(dt)*Z), dt=1
    b <- beta_t * exp((mu - 0.5 * sig^2) + sig * rnorm(1))
    return(.clamp_beta(b, parameters))
  }
...
run_sir_binomial <- function(initial_state,
  ...
  for(i_day in times[-1]){
    beta_t <- evolve_beta(beta_t, parameters, Ns, Ni, bool_stochastic_beta)
    #  beta_t <- parameters$beta
   ...
   # a_t <- a_function(Ni, Ns, parameters)
    ...
    # }
    if (isTRUE(parameters$bool_daily_cost_minimizing)) {
      a_t <- a_function_cost_minimizing(Ns, Ni, i_day, parameters)  # pass beta_t here too if you modify that function
    } else {
      a_t <- a_function(Ni, Ns, parameters, beta_t = beta_t)
    }
    
    R0_t <- beta_t / parameters$gamma
    Rt   <- calculate_Rt(R0_t, a_t, Ns/parameters$pop_size, Ni)
