...
a_susceptible <- function(Ni, parameters, A_I = 1) {
  Ni_prop <- Ni / parameters$pop_size
  term <- parameters$beta * parameters$kappa * A_I * Ni_prop
  aS <- 1 / (1 + term)
  return(max(0, min(1, aS)))
}

# Infected & recovered (baseline: no altruism/penalty)
a_infected  <- function(parameters)  1
a_recovered <- function(parameters)  1
...
#   #  return(-1/2 * (1 - a_t)^2)
# }
# --- Helper functions for separate S/I activities ---

# Safe utility function (avoids log(0) errors)
utility_of_activity <- function(a) {
  a <- max(1e-8, min(1, a))  # clamp to [ε, 1]
  log(a) - a + 1
}

# Rt calculation when susceptibles and infecteds have separate activities
calculate_Rt_two_groups <- function(R0, aS, aI, Ns_prop, Ni) {
  if (Ni <= 0) return(0)
  R0 * aS * aI * Ns_prop
}
...
run_sir_binomial <- function(initial_state,
  ...
  # Output matrix
  states_out <- matrix(NA, nrow = length(times), ncol = 12)
  ...
  for (i_day in times[-1]) {
    ...
    # IFR adjustment if capacity exceeded
    ...
    # --- Activities (full information) ---
    aI <- a_infected(parameters)
    aS <- a_susceptible(Ni, parameters, A_I = aI)
    aR <- a_recovered(parameters)
    uS <- utility_of_activity(aS)
    
    # Infection & transition probabilities
    p_infect  <- 1 - exp(- beta_t * aS * aI * Ni / parameters$pop_size)
    ...
    if (!isTRUE(parameters$bool_regular_sird)) {
      ...
      HealthCost <- HealthCost + fx_per_capita * disc * parameters$v * new_death
      SocialActivityCost <- SocialActivityCost + fx_per_capita * disc * Ns * abs(uS)
    }
    
    # Rt
    Rt <- calculate_Rt_two_groups(parameters$R0, aS, aI, Ns / parameters$pop_size, Ni)
...
compare_sim_output <- function(output_experiments, output_deterministic,
  ...
  pretty_labels <- c(
    "a_S" = "Activity (Susceptible)",
    "a_I" = "Activity (Infected)",
    "a_R" = "Activity (Recovered)",
    "Ni" = "Infectives",
    "Ns" = "Susceptibles",
    "Nr" = "Recovered",
    "Nd" = "Deceased",
  ...
  a_det <- output_deterministic[t_peak, "a_S"]
