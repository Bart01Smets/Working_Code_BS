...
sir_costate_model <- function(time, state, parameters) { 
  with(as.list(c(state, parameters)), { # this statements enables the use of the parameter and state names
    ...
    return(list(c(dNs, dNi, dNr, dNd, dLambda_s, dLambda_i, dHealthCost, dSocialActivityCost, dTotalCost, a_t, u_a, R_t)))
  })
}
...
compute_terminal_conditions <- function(ns_T, ni_T, parameters) {
  ...
  return(c(Lambda_s_T, Lambda_i_T))
}
...
initial_state <- c(Ns = parameters$ns0, 
                   ...
                   Nr = parameters$nr0, 
                   Nd = parameters$nd0,
                   Lambda_s = -102.859,#Lambda_values[1], -102.859	-194.343
                   Lambda_i = -194.343,#Lambda_values[2], -165.651	-21985.5
                   HealthCost = 0, 
...
#print(output_intervention_df$Ni)
#write.table(output_intervention_df$ni, "clipboard", sep = "\t", row.names = FALSE, col.names = FALSE)
#output_intervention_df_inf<-output_intervention_df$Ni
...
# Create a data frame with time, activity (a_t), and utility (u_t)
#activity2_utility_df <- data.frame(
#  Time = output_intervention_df$time,
 # Activity = output_intervention_df$a_t,
#  Utility = output_intervention_df$u_t
#)

# Export to Excel file
#write_xlsx(activity2_utility_df, "Activity2_Utility_Through_Time.xlsx")

# Also, export as CSV if needed
#write.csv(activity2_utility_df, file = "Activity2_Utility_Through_Time.csv", row.names = FALSE)

# Print confirmation message
cat("Activity and Utility data successfully exported to 'Activity_Utility_Through_Time.xlsx' and CSV format.\n")


#df <- data.frame(Value = output_intervention_df$Ni)
#write.csv(df, file = "output_intervention_df_2_ni.csv", row.names = FALSE)
#df6 <- data.frame(Value = output_intervention_df$a_t)
#write.csv(df6, file = "output_intervention_df_6_at.csv", row.names = FALSE)
