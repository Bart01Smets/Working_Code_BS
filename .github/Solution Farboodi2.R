...
evaluation_times <- c(
  0.000071152, 0.000142304, 0.000213456, 0.000284608, 0.000355576, 0.000498064,
  0.000640368, 0.00206341, 0.00348645, 0.00490949, 0.0191399, 0.0333703,
  0.0476007, 0.189905, 0.332209, 0.474513, 1.89755, 3.32059, 4.74363,
  12.0241, 19.3046
)

# Your desired output times (e.g., days 1 through 20)
#desired_output_times <- 1:20

# Combine and sort them into a unique time vector
#all_times <- sort(unique(c(evaluation_times, desired_output_times)))
       ...
       #                    times = time_pre_shock, 
         ...
         #                  method="lsoda"
                           output_intervention <- ode(
                             ...
                             func = sir_costate_model, 
                             parms = parameters,
                             method = "rk4"   # lsoda automatically handles stiff/non-stiff transitions
...
#) #tcrit = time_pre_shock)
                           ...
                           df_custom <- as.data.frame(output_intervention)
                           # Desired integer times
                           desired_times <- 1:20
                           df_report <- data.frame(time = desired_times)
                           
                           # Prepare a data frame for the interpolated results
                           df_report <- data.frame(time = desired_times)
                           
                           # For each state variable, interpolate from df_custom
                           df_report$Ni <- approx(x = df_custom$time, y = df_custom$Ni,
                                                  xout = desired_times)$y
                           df_report$Ns <- approx(x = df_custom$time, y = df_custom$Ns,
                                                  xout = desired_times)$y
                           df_report$Nr <- approx(x = df_custom$time, y = df_custom$Nr,
                                                  xout = desired_times)$y
                           df_report$a_t  <- approx(x = df_custom$time, y = df_custom$a_t,  xout = desired_times, rule = 2)$y
                           # etc. for any other variables
                           ...
                           output_df <- as.data.frame(output_intervention)
                           
                           # Subset the results at your desired output times
                           output_at_desired <- output_df[output_df$time %in% desired_times, ]
                          print(output_at_desired) 
