...
parameters <- list(
  ...
  age_shares = c(0.7, 0.3),              # population shares (sum ≈ 1)
  
  # If omitted, these fall back to scalar pi / v
  pi_by_age = c(Y = 0.001, O = 0.02),    # age-specific IFRs
#  v_by_age  = c(Y = 10000, O = 50000),   # age-specific value of life/cost
  beta_matrix = NULL,                     # full GxG matrix β_{g,h}; rows = susceptible g, cols = infected h
  beta_susc_by_age = c(Y = 1.0, O = 1.0),# optional susceptibility multipliers (length G)
  beta_inf_by_age  = c(Y = 1.0, O = 1.0),# optional infectiousness multipliers (length G)
  gamma_by_age     = c(Y = 1/7, O = 1/7), # age-specific recovery rates (length G)
  # Mixing matrix (rows = susceptible group, cols = infected group)
  # Each row should sum to ~1; defaults to assortative mixing
  contact_matrix = matrix(c(0.8, 0.2,
                            0.2, 0.8),
                          nrow = 2, byrow = TRUE,
                          dimnames = list(c("Y","O"), c("Y","O"))),
  
  # Optional: initial infected shares by age (defaults to age_shares if NULL)
  infect_init_shares = NULL,
  
  # Optional: do groups choose separate activity levels (default = single common a_t)
  bool_age_specific_activity = TRUE
