...
# Single-run wrapper (works for either stochastic-beta or binomial randomness)
run_one_simulation <- function(initial_state, times, parameters,
                               bool_stochastic_beta = FALSE,
                               update_function = get_transitions_stochastic) {
  run_sir_binomial(initial_state, times, parameters,
                   bool_stochastic_beta = bool_stochastic_beta,
                   update_function = update_function)
}

# Simple overlay plot: stochastic vs deterministic (no bands)
plot_sim_vs_det <- function(output_stochastic, output_deterministic, plot_tag = "single-stochastic") {
  if (!dir.exists("figures")) dir.create("figures")
  pdf(file = paste0("figures/", gsub("[^[:alnum:]]", "_", plot_tag), ".pdf"))
  
  vars <- intersect(c("a_t","Ni","Ns","Nr","Nd","u_t","Rt"),
                    intersect(names(output_stochastic), names(output_deterministic)))
  time_vec <- seq_len(nrow(output_deterministic)) - 1
  ...
  for (v in vars) {
    y_rng <- range(output_stochastic[[v]], output_deterministic[[v]], na.rm = TRUE)
    plot(time_vec, output_deterministic[[v]], type = "l", lty = 2, lwd = 2,
         ylim = y_rng, xlab = "time", ylab = v, main = v)
    lines(time_vec, output_stochastic[[v]], lwd = 2)
  ...
  # Console summary
  det_final   <- tail(output_deterministic, 1)
  stoch_final <- tail(output_stochastic, 1)
  cat("\n=== Final (per capita) costs ===\n")
  cat(sprintf("Deterministic: Health=%.0f  Social=%.0f  Total=%.0f\n",
              det_final$HealthCost, det_final$SocialActivityCost, det_final$TotalCost))
  cat(sprintf("Stochastic:    Health=%.0f  Social=%.0f  Total=%.0f\n",
              stoch_final$HealthCost, stoch_final$SocialActivityCost, stoch_final$TotalCost))
  
  det_peak   <- which.max(output_deterministic$Ni) - 1
  stoch_peak <- which.max(output_stochastic$Ni) - 1
  cat(sprintf("\nPeak time (Ni):  Deterministic = %d,  Stochastic = %d\n", det_peak, stoch_peak))
}
