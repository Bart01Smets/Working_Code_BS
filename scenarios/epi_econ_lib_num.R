...
#   if (Ni_prop < 1e-10 || Ns_prop < 1e-10) return(1)
# 
#   # Constant in FOC after symmetry
#   c_t <- parameters$beta * parameters$pi * parameters$v * Ns_prop * Ni_prop
# 
#   # Exact-discrete-probability FOC:
#   # (1/a - 1) - c_t * a * exp(-parameters$beta * a^2 * Ni_prop) = 0
#   foc <- function(a) {
#     (1 / a - 1) - c_t * a * exp(-parameters$beta * a^2 * Ni_prop)
#   }
# 
#   # Solve for a in (0,1]
#   a_t <- tryCatch(
#     uniroot(foc, interval = c(1e-8, 1), tol = 1e-10)$root,
#     error = function(e) 1  # fallback to 1 if no root
#   )
# 
#   return(max(0, min(1, a_t)))
# }
# 
# a_function <- function(Ni, Ns, parameters) {
#   # Always full activity for regular SIRD
...
#   
#   # Constant in FOC
#   # Note: parameters$pi * parameters$v = κ
#   C_t <- (2 * parameters$beta * parameters$pi * parameters$v * Ns_prop * Ni_prop) /
#     (Ns_prop + Ni_prop)
#   
#   # FOC: (1/A - 1) - C_t * A * exp(-beta * A^2 * Ni_prop) = 0
#   foc <- function(A) {
#     (1 / A - 1) - C_t * A * exp(-parameters$beta * A^2 * Ni_prop)
#   }
#   
