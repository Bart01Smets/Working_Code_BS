...
run_sir_binomial <- function(initial_state,
  ...
  for(i_day in times[-1]){
    ...
    # # compute tentative next infected
    # Ni_next <- Ni + new_infections - new_recoveries
    # 
    # # fade-out adjustment (apply BEFORE deltas & costs)
    # if (!is.null(parameters$infect_thres) && Ni_next < parameters$infect_thres) {
    #   # snap to zero infections: no new infections this step,
    #   # move all current infecteds to recovered, and no deaths in this last step
    #   new_infections <- 0
    #   new_recoveries <- Ni
    #   new_death      <- 0
    #   Ni_next        <- 0
