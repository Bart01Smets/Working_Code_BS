...
# load functions
source("scenarios/epi_econ_lib_RA.R")
...
parameters <- list(
  ...
  second_wave_day = NULL,                 # day the second wave starts700
  second_wave_beta_multiplier = NULL, # +50% transmissibility1.5
  # --- Risk preferences (for stochastic runs) ---
  bool_risk_averse = TRUE,   # FALSE = risk-neutral (current behavior)
  risk_aversion    = 2,      # γ > 0 (e.g., 1 = log, 2 = moderate)
  risk_type        = "CRRA",  # "CRRA" or "expo"
...
# Build per-day reference cost series from cumulative columns
det_ref <- diff(output_sim_deterministic$HealthCost) +
  diff(output_sim_deterministic$SocialActivityCost)
# times[-1] days correspond to det_ref
parameters$ra_ref_series <- c(0, det_ref)  # pad to align index i_day+1
...
# <<< INSERT HERE >>>
det_final <- tail(output_sim_deterministic$WelfareRA, 1)
stoch_final <- output_experiments$output_summary$WelfareRA
cat("Deterministic RA welfare:", det_final, "\n")
cat("Stochastic RA welfare (mean):", mean(stoch_final), "\n")
cat("95% CI:", ci_mean(stoch_final)$interval, "\n")
