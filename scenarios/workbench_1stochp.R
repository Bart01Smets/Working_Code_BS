...
# === Single stochastic run (binomial transitions) + PDF ===
num_experiments <- 1
set.seed(parameters$rng_seed)
...
output_experiments <- run_experiments(
  ...
  times = times,
  parameters = parameters,
  bool_stochastic_beta = FALSE,          # set TRUE if you also want random beta_t
  update_function = get_transitions_stochastic,
  num_experiments = num_experiments
)

compare_sim_output(output_experiments, output_sim_deterministic, plot_tag = "one_stoch")

out_stoch <- run_sir_binomial(
  initial_state = initial_state,
  times = times,
  parameters = parameters,
  bool_stochastic_beta = FALSE,          # set TRUE if you also want random beta_t
