...
for(i_exp in 1:num_experiments){
  ...
  # Example with stochastic beta
 # parameters_exp$beta <- parameters_exp$beta + parameters$sigma * rnorm(1, mean = 0, sd =3 )#sqrt(1)
...
# parameters_exp$beta <- parameters_exp$beta + rnorm(1,mean=0,sd=1e-3)
  ...
  # Initialize stochastic beta
  #beta_t <- parameters_exp$beta
  #beta_values <- numeric(length(time_pre_shock)) # To store stochastic beta values
  
  #for (t in seq_along(time_pre_shock)) {
    # Add stochastic term to beta
   # dZ_t <- rnorm(1, mean = 0, sd = sqrt(dt)) # Standard Brownian motion
  #  beta_t <- beta_t + parameters$sigma * dZ_t
  #  beta_values[t] <- beta_t # Store the value for tracking
  #}
  
  # Use the last stochastic beta for the run
 # parameters_exp$beta <- beta_values
