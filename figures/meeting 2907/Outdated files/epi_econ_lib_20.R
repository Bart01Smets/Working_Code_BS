...
# New a_function based on R_t feedback control
a_function <- function(Rt, parameters) {
  sensitivity <- parameters$rt_sensitivity  # Controls how sharply a_t reacts to Rt > 1

  if (is.na(Rt) || Rt < 0) {
    warning("Invalid Rt input to a_function; returning a_t = 1")
    return(1)
  }

  # Reduce a_t only if Rt > 1
  a_t <- 1 / (1 + sensitivity * max(0, Rt - 1))
...
run_sir_binomial <- function(initial_state, 
  ...
  for(i_day in times[-1]){
    ...
    #a_t <- a_function(Ni, parameters)
    # Estimate Rt from previous step (use a_t = 1 for time step 1)
    Rt_input <- if (i_day == times[2]) {
      calculate_Rt(parameters$R0, 1, Ns / parameters$pop_size)
    } else {
      Rt
    }
    a_t <- a_function(Rt_input, parameters)
...
compare_sim_output <- function(output_experiments, output_deterministic, 
 ...
 # par(mfrow=c(3,4), bg = "white")
  ...
  for(i_state in sel_states){
    ...
    # Force white background only for the problematic plots
    if (i_state %in% c("a_t", "u_t", "Rt")) {
      plot(output_sim_deterministic[,i_state], col=1, main=i_state, ylab = i_state, xlab = 'time',
           ylim = y_lim, type='l', lwd=2, bg="white")
    } else {
      plot(output_sim_deterministic[,i_state], col=1, main=i_state, ylab = i_state, xlab = 'time',
           ylim = y_lim, type='l', lwd=2)
    }
    
    for(i_exp in 1:dim(output_all)[1]){
      output_sim <- data.frame(output_all[i_exp,,i_state])
      lines(output_sim, col=alpha(2, 0.3)) # stochastic
    }
    lines(output_sim_deterministic[,i_state], col=1, lwd=2) # deterministic
  }
