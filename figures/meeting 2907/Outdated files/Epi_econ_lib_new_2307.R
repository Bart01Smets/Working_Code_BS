...
# Binomial transitions (preserved)
...
# Kernel function for binomial SIRD simulation

run_sir_binomial <- function(initial_state, times, parameters,
  ...
  for(i_day in times[-1]){
    beta_t <- parameters$beta  # Fixed beta
    ...
    scale_factor <- 1
    HealthCost <- HealthCost + scale_factor * fx_per_capita * exp(-rho_plus_delta * i_day) * parameters$kappa * Ni * parameters$gamma
    SocialActivityCost <- SocialActivityCost + scale_factor * fx_per_capita * exp(-rho_plus_delta * i_day) * (Ns + Ni) * abs(u_t)
    Rt <- calculate_Rt(parameters$R0, a_t, Ns / parameters$pop_size, Ni)
    
    Ns <- Ns + dNs; Ni <- max(Ni + dNi, 0)
    Nr <- Nr + dNr; Nd <- Nd + dNd
...
compare_sim_output <- function(output_experiments, output_deterministic, 
  ...
  for(i_state in sel_states){
    ...
    for(i_exp in 1:dim(output_all)[1]){
      ...
      lines(output_sim,col=alpha(2,0.3)) 
    }
