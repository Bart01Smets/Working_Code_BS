...
#Original welfare function
# a_function <- function(Ni, Ns, parameters) {
#   Ni_prop <- Ni / parameters$pop_size
#   Ns_prop <- Ns / parameters$pop_size
# 
#   multiplier <- parameters$beta *Ni_prop*Ns_prop*parameters$kappa#*(1 + parameters$alpha* Ns_prop) (In case of optimal policy)
#   sqrt_term <- sqrt(1 + 8 * multiplier) #_prop
#   a_t <- (-1 + sqrt_term) / (4 * multiplier)
#   if (Ni < 1e-10 || Ns < 1e-10) return(1)
#   return(max(0, min(1, a_t)))
# }
#Welfare function used; with Aa and without (ns+ni)u(a))
a_function <- function(Ni, Ns, parameters) {
  Ni_prop <- Ni / parameters$pop_size
  Ns_prop <- Ns / parameters$pop_size
  
  num_sum <- Ns_prop + Ni_prop
  mult <- parameters$beta * parameters$kappa * Ns_prop * Ni_prop#*parameters$gamma
  denom <- 2 * mult
...
#Welfare function with Aa and with (ns+ni)u(a))
# a_function <- function(Ni, Ns, parameters) {
...
#   multiplier <- parameters$beta * parameters$kappa * Ns_prop * Ni_prop
#   num_sum <- Ns_prop + Ni_prop
...
run_sir_binomial <- function(initial_state, 
  ...
  for(i_day in times[-1]){
    ...
    new_infections <- update_function(Ns, prob = beta_t * a_t^2 * Ni/parameters$pop_size)#/parameters$pop_size
    ...
    scale_factor <- 1
    HealthCost <-  HealthCost+ scale_factor* fx_per_capita * exp(-rho_plus_delta * i_day) *  parameters$kappa  * Ni* parameters$gamma#/parameters$pop_size#HealthCost + parameters$kappaNd/parameters$pop_size*parameters$v
    SocialActivityCost <- SocialActivityCost+ scale_factor* fx_per_capita * exp(-rho_plus_delta * i_day) * (Ns + Ni) * abs(u_t)#/parameters$pop_size#SocialActivityCost +
...
compare_sim_output <- function(output_experiments, output_deterministic, 
  ...
  if (!dir.exists("figures")) dir.create("figures")  # should go before pdf()
  
  pdf(file = paste0("figures/", gsub("[^[:alnum:]]", "_", plot_tag), ".pdf"))
  
  # --- Start plotting here ---
  # boxplots, plots, lines(), etc.
