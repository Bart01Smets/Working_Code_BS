...
sir_costate_model <- function(time, state, parameters) { 
  with(as.list(c(state, parameters)), {
   ...
   # dLambda_i <- (rho + delta) * Lambda_i - (u_t + alpha * (Lambda_i - Lambda_s) * beta * a_t^2 * Ns) + gamma * (kappa + Lambda_i)
    dLambda_s <- (rho + delta)* Lambda_s - 
      u_t - (Lambda_i - Lambda_s) * beta * a_t^2 * Ni 
    dLambda_i <- (rho + delta) * Lambda_i - 
      u_t - (Lambda_i - Lambda_s) * beta * a_t^2 * Ns  + 
      gamma * (kappa + Lambda_i)
    ...
    #dHealthCost <- (rho_plus_delta * HealthCost) - (parameters$gamma * parameters$kappa * Ni)
    #HealthCost <- HealthCost + dHealthCost * time
    
    # Update SocialActivityCost
    #dSocialActivityCost <- (rho_plus_delta * SocialActivityCost) + ((Ns + Ni) * abs(u_t))
    #SocialActivityCost <- SocialActivityCost + dSocialActivityCost * time
