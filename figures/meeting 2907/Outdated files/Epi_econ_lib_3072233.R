...
#  lambda_i <- 0
 ...
 #   a_t <- a_function(Ni, parameters$alpha, beta_t, 
  ...
  #  a_t <- a_function(Rt, parameters)
    ...
    Ns_prop <- Ns / parameters$pop_size
    a_t <- a_function(Ni, Ns, parameters)#Nd, v
#Rt parameters Ni Rt a_t_prev
  ...
  #    d_lambda_i <- rho_plus_delta * lambda_i - 
   #     u_t - (lambda_i - lambda_s) * beta_t * a_t^2 * Ns / parameters$pop_size - 
    ...
    scale_factor <- 1
    HealthCost <-  HealthCost+ scale_factor* fx_per_capita * exp(-rho_plus_delta * i_day) *  parameters$kappa  * Ni#/parameters$pop_size#HealthCost + parameters$kappaNd/parameters$pop_size*parameters$v
  ...
  #  HealthCost <- HealthCost + HealthCostDaily
    ...
    states_out[i_day+1,] = c(Ns, Ni, Nr, Nd,
                             ...
                             HealthCost + SocialActivityCost,
                             a_t, u_t, Rt)# HealthCostDaily, SocialActivityCostDaily)#Lambda_s, Lamba_i
  }
...
compare_sim_output <- function(output_experiments, output_deterministic, 
 ...
 # sel_states <- #c("HealthCostDaily", "SocialActivityCostDaily",
  ...
  # Friendly label mapping for selected variables
  pretty_labels <- c(
    "a_t" = "Activity",
    "Ni" = "Infectives",
    "Ns" = "Susceptibles",
