...
parameters <- list(
  ...
  pi = 0.0062,             # Infection fatality rate (π)
    sigma=1.689,#1.689,            # Determines size of drift term ()
...
# some graphical exploration
par(mfrow=c(2,2)) # reset sub-panelsTotalActivityCost
xx <- boxplot(output_experiments$HealthCost,main='HealthCost')
abline(h=output_sim_deterministic$HealthCost[nrow(output_sim_deterministic)],col=4)
text(x=0.6,y=output_sim_deterministic$HelathCost[nrow(output_sim_deterministic)],
