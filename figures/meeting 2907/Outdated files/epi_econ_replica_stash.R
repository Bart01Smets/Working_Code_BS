...
a_function <- function(alpha, beta, Ns, Ni, lambda_s, lambda_i, tolerance = 1e-8) {
  denom <- max(abs(lambda_i - lambda_s), tolerance)  # prevent division by zero
  
  if (Ni == 0 || Ns == 0) return(1)
  
  num_pop <- Ns + Ni
  sqrt_arg <- num_pop^2 + 8 * beta * Ns * Ni * denom * (1 + alpha) * num_pop
  
  if (sqrt_arg < 0) return(1)  # guard against negative sqrt
  
  numerator <- -num_pop + sqrt(sqrt_arg)
  denominator <- 4 * beta * Ns * Ni * denom * (1 + alpha)
  
  a_t <- numerator / denominator
  return(max(min(a_t, 1), 0))  # constrain to [0,1]
}
...
solve_for_a <- function(Ns, Ni, lambda_s, lambda_i, beta, rho_plus_delta, t, dt, utility_type) {
  foc_function <- function(a) {
    if (utility_type == "Log") {
      u_prime <- 1 / a - 1
    } else {
      stop("Only Log utility supported.")
    }
    exp_term <- exp(-rho_plus_delta * t * dt)
    return(exp_term * (Ns + Ni) * u_prime + beta * a * Ns * Ni * (lambda_i - lambda_s))
  }
  ...
  result <- tryCatch({
    uniroot(foc_function, interval = c(1e-4, 1))$root
  }, error = function(e) {
    return(1)
  })
...
run_sir_binomial <- function(initial_state, 
  ...
  for(i_day in times[-1]){
    ...
    #                  Ns/parameters$pop_size, Ni/parameters$pop_size, 
    #                  lambda_s, lambda_i, 
    #                 parameters$utility_type, parameters$scenario, parameters$tolerance)
    
    # a_t <- solve_for_a(
    #   Ns = Ns / parameters$pop_size,
    #   Ni = Ni / parameters$pop_size,
    #   lambda_s = lambda_s,
    #   lambda_i = lambda_i,
    #   beta = beta_t,
    #   rho_plus_delta = rho_plus_delta,
    #   t = i_day,
    #   dt = parameters$dt,
    #   utility_type = parameters$utility_type
    # )
    
    a_t <- a_function(parameters$alpha, beta_t, Ns / parameters$pop_size, Ni / parameters$pop_size,
                      lambda_s, lambda_i, parameters$tolerance)
    ...
    if (parameters$scenario == "laissez-faire") {
      ...
      # A_t <- a_t  # aggregate activity; put equal to regular a_t for now
      # disc <- exp(-rho_plus_delta * i_day * parameters$dt)
      # 
      # lambda_s_new <- disc * u_t +
      #   beta_t * a_t * A_t * Ni / parameters$pop_size * (lambda_i - lambda_s)
      # 
      # lambda_i_new <- disc * (u_t - parameters$gamma * parameters$kappa) +
      #   beta_t * a_t * A_t * Ns / parameters$pop_size * (lambda_i - lambda_s) -
      #   parameters$gamma * lambda_i
      # 
      # lambda_s <- lambda_s_new
      # lambda_i <- lambda_i_new
      
      A_t <- a_t  # Aggregate activity = representative agent activity
      disc <- exp(-rho_plus_delta * i_day * parameters$dt)
      
      lambda_s_new <- disc * u_t +
        (1 + parameters$alpha) * beta_t * a_t * A_t * Ni / parameters$pop_size * (lambda_i - lambda_s)
      
      lambda_i_new <- disc * (u_t - parameters$gamma * parameters$kappa) +
        (1 + parameters$alpha) * beta_t * a_t * A_t * Ns / parameters$pop_size * (lambda_i - lambda_s) -
        parameters$gamma * lambda_i
    ...
    #    d_lambda_i <- rho_plus_delta * lambda_i + 
    #     parameters$gamma * parameters$kappa + 
    #     beta_t * a_t^2 * Ns / parameters$pop_size * (parameters$alpha + lambda_s - lambda_i) + 
    #    parameters$gamma * lambda_i
...
compare_sim_output <- function(output_experiments, output_deterministic, 
  ...
  sel_states <- names(initial_state)[
    !grepl('Total', names(initial_state)) &
      !grepl('Ns', names(initial_state)) &
      !grepl('lambda_', names(initial_state))  #  Exclude lambda_s and lambda_i
  ]
