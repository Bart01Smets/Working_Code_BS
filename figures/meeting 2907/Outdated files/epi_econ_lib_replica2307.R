...
a_function <- function(alpha, beta, Ns, Ni, lambda_s, lambda_i, utility_type, scenario, tolerance) {
  ...
  if (denom < tolerance || Ns == 0 || Ni == 0) {
    return(1)
  }
  
  if (scenario == "laissez-faire") {
    sqrt_arg <- (Ns + Ni)^2 + 4 * beta * denom * Ns * Ni * (Ns + Ni)
    a_star <- (-(Ns + Ni) + sqrt(sqrt_arg)) / (2 * beta * denom * Ns * Ni)
    return(max(0, min(1, a_star)))
  }   else if (scenario == "optimal-policy") {
    C_t <- 4 * beta * exp((parameters$rho + parameters$delta) * parameters$time) * 
      Ns * Ni * max(lambda_i - lambda_s, tolerance)
    sqrt_arg <- (Ns + Ni)^2 + C_t * (Ns + Ni)
    a_star <- (-(Ns + Ni) + sqrt(sqrt_arg)) / C_t
    return(max(0, min(1, a_star)))
  
  
 
  } else if (scenario == "altruism") {
    multiplier <- (1 + alpha)
    sqrt_arg <- (Ns + Ni)^2 + 8 * multiplier * beta * denom * Ns * Ni * (Ns + Ni)
    a_star <- (-(Ns + Ni) + sqrt(sqrt_arg)) / (4 * multiplier * beta * denom * Ns * Ni)
    return(max(0, min(1, a_star)))
  }
  
  # fallback in case of unknown scenario
  return(1)
}
...
run_sir_binomial <- function(initial_state, 
  ...
  for(i_day in times[-1]){
    ...
    if (parameters$scenario == "laissez-faire") {
      d_lambda_s <- rho_plus_delta * lambda_s - u_t + 
        beta_t * a_t^2 * Ni / parameters$pop_size * (lambda_s - lambda_i)
      
      d_lambda_i <- rho_plus_delta * lambda_i - u_t + 
        beta_t * a_t^2 * Ns / parameters$pop_size * (lambda_s - lambda_i)
      
    
    } else if (parameters$scenario == "optimal-policy") {
      
      d_lambda_s <- exp(-rho_plus_delta) * (
        utility_function(a_t, parameters$utility_type) -
          beta_t * a_t^2 * (Ni / parameters$pop_size) * (lambda_s - lambda_i)
      ) - lambda_s
      
      d_lambda_i <- exp(-rho_plus_delta) * (
        utility_function(a_t, parameters$utility_type) -
    ...
    } else if (parameters$scenario == "altruism") {
      multiplier <- 1 + parameters$alpha
      
      d_lambda_s <- rho_plus_delta * lambda_s - u_t + 
        multiplier * beta_t * a_t^2 * Ni / parameters$pop_size * (lambda_s - lambda_i)
      
      d_lambda_i <- rho_plus_delta * lambda_i - u_t + 
        multiplier * beta_t * a_t^2 * Ns / parameters$pop_size * (lambda_s - lambda_i) + 
